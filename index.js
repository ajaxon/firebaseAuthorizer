...
admin.initializeApp({
    ...
    databaseURL: "https://userpool-1d1e0.firebaseio.com"
});
...
function verifyFireBaseToken(token, callback){
    console.log("Token: ", token);
    admin.auth().verifyIdToken(token)
        .then(function (decodedToken) {
            console.log("User ID ", decodedToken.uid);
            //let token = generateJwt(decodedToken);


        }).catch(function (error) {
        console.log("An error occured on verify: ", error);
        let responseBody = {
...
exports.handler = function(event, context, callback) {
    ...
    admin.auth().verifyIdToken(token)
        .then(function (decodedToken){
            ...
            generatePolicy(callback,event, principalId);
...
function generatePolicy(callback, event, principalId){
